...
type UserRole = "farmer" | "veterinary" | "consumer" | "retailer" | "delivery" | "admin";

function routeForSuggestion(role: UserRole | null, suggestion: Suggestion): string {
  ...
  const raw = suggestion.url?.startsWith("/") ? suggestion.url : `/${suggestion.url || ""}`;
  const clean = raw.split("?")[0].split("#")[0];

  if (role === "farmer" && (clean === "/marketplace" || clean.startsWith("/marketplace/"))) {
    if (suggestion.type === "product") return "/farmer/equipment";
    if (suggestion.type === "market") return "/farmer/trading";
    return "/farmer";
  }

  const allowedPrefixes: Record<UserRole, string[]> = {
    farmer: ["/farmer", "/veterinary/map", "/track/", "/invoice/"],
    veterinary: ["/veterinary", "/track/", "/invoice/"],
    consumer: ["/marketplace", "/consumer", "/track/", "/invoice/"],
    retailer: ["/retailer", "/track/", "/invoice/"],
    delivery: ["/delivery", "/track/", "/invoice/"],
    admin: ["/admin", "/track/", "/invoice/"],
  };

  if (!role) return clean || "/";
  return allowedPrefixes[role].some((prefix) => clean === prefix || clean.startsWith(prefix.endsWith("/") ? prefix : `${prefix}/`))
    ? raw
    : roleHome[role];
}
...
async function getSuggestionsForUser(
  ...
  const [farmsRes, detectionsRes, ordersRes, recsRes, productsRes, interactionsRes, roleRes] = await Promise.all([
  ...
  const role = ((roleRes.data as { role?: UserRole } | null)?.role ?? null);
...
Return JSON:
{
  "suggestions": [
    {
      ...
      "type": "weather|disease|market|scheme|product|general",
      "url": "/farmer/weather | /farmer/disease | /marketplace | /farmer/schemes | /farmer",
      "score": 0-100
    }
  ]
}`;
  ...
  try {
    ...
    return suggestions
      ...
      .slice(0, 3)
      .map((s) => ({ ...s, url: routeForSuggestion(role, s) }));
  } catch (e) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Authorization: derive user from JWT for single-user calls; require service-role for digest
    ...
    let userIds: string[] = [];
    if (!isServiceCall) {
      // Authenticated user — they can only request suggestions for themselves
      if (!token) {
